...
- [ ] 1.1 Replace raw-object ledger authority and
  ...
  ledger entry.
- [ ] 1.2 Add trusted fold/capture/adoption capabilities: only explicit carrier
  or ref adoption reuses history; recapturing the same raw value starts an
  independent operation.
- [ ] 1.3 Keep `events` strictly sorted by unique immutable `order`; retain
  semantic primacy through `ledger.primary`, not event-vector reordering.
- [ ] 1.4 Make semantic-primary access branch on ledger presence so exact
  `null`, `undefined`, falsy, private-field, and external-WeakMap-branded raw
  values round-trip unchanged.
- [ ] 1.5 Prove sequential, `Promise.all` concurrent, and getter-reentrant reuse
...
- [ ] 2.1 Retain iterative 4096-node/8192-edge limits and add pre-charged
...
- [ ] 2.2 Replace unbounded `instanceof`/prototype traversal with bounded brand
...
- [ ] 2.3 Call `Reflect.ownKeys()` at most once per observed array/container;
...
- [ ] 2.4 Model array-brand observation as `array | non_array | failed`; on
  failure record exactly one occurrence and add no child edge/node.
- [ ] 2.5 Retain N-1/N/N+1 node/edge, max-length sparse array, 25K+ chain,
...
- [ ] 3.1 Brand genuine `TaskServiceError` construction with private
  capability state; stop treating prototype traversal as authenticity.
- [ ] 3.2 Provide a controlled trusted Proxy factory that privately binds the
  Proxy to a branded target. Do not expose arbitrary proxy/target registration.
- [ ] 3.3 Store the verified typed target as a private carrier view. Backend and
  core consumers resolve this view before raw semantic unwrapping and do not
  touch/reclassify a one-shot Proxy twice.
- [ ] 3.4 Core negative matrix: field-shaped, `Object.create(prototype)`,
...
- [ ] 3.5 Backend matrix: every negative input maps to generic 500; positives
...
- [ ] 4.1 Make `runWithPreservedRelease` and typed adapters accept/produce
...
- [ ] 4.2 Migrate all relevant generic TaskCard and backend compensation/release
  producers and consumers to the carrier/accessor contract.
- [ ] 4.3 Migrate the shared `idempotency-service.ts` release wrapper and the
...
- [ ] 4.4 Audit manual TaskCard close/release folds and phase arrays so capture
  happens at the physical catch site and final-release errors are not labelled
  `body` or `settlement`.
- [ ] 4.5 Add real-producer regression rows for all available TaskCard,
...
- [ ] 5.1 Add replayable red-before evidence for the new carrier isolation,
...
- [ ] 5.2 Run the dedicated core/backend ledger tests and every test touched by
  TaskCard/idempotency/workspace/backend migration.
- [ ] 5.3 Run the full core and backend suites, typecheck,
...
- [ ] 5.4 Run Phase 6.2 invariant audit across shared helper roots, public
...
- [ ] 5.5 Bind exact final commands, pass/fail/skip counts, red evidence paths,
  file hashes, and source/test diff hash into tracked verification evidence.

## 6. Scope and Oracle Integrity

- [ ] 6.1 No dependency, persisted schema, public HTTP payload, Zero submodule,
  private transition-ticket/generation settlement, or watcher change.
- [ ] 6.2 Existing tests/spec/CI gates are not deleted or weakened to pass.
...
- [ ] 6.3 Every Round-1 confirmed finding P1/P2/S1/S2/S3/C1/C2/C3/TE1/TE2 is
