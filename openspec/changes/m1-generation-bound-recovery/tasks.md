...
- [x] 1.1 Add public-service red-before/green-after races at two exact service seams: (a) create a stale persisted fail-intent guard, call `lookupReplay`, pause after its deciding exact observation of started A and before `replaceJsonRecordAfterExactObservation` is invoked, install byte-distinct same-digest completed B, then require `completed` B; (b) call `recoverCompletedRecordAfterRollbackFailure`, pause after its deciding exact observation of A and before exact replacement is invoked, install B, then require returned B. In both rows replay returns B, persisted bytes equal B exactly, the stale guard is settled when owned, and guard/permit/authority diagnostics equal the pre-call baseline.
- [x] 1.2 Bind both recovery writers to the single exact observation that makes the transition decision using existing `observeJsonRecordForCleanup` and `replaceJsonRecordAfterExactObservation`; preliminary replay reads are semantic only, no observation after the deciding exact observation may become writable authority, and no record-store publication primitive/carrier/provenance API may be added or modified.
- [x] 1.3 At the pre-invocation seam for stale-guard `lookupReplay`, prove separately: unchanged A -> `incomplete` failed record and stale guard consumed; same-digest valid completed B -> `completed`; different-digest B -> `mismatch`; missing/unsafe-result completed B -> `invalid_completed`; byte-distinct same-digest started or failed B, including a canonical-equal raw-byte/physical replacement -> `incomplete` B with the guard for lost A consumed; malformed bytes B -> rejection with `TaskServiceError.code=record_malformed`; missing successor -> `missing`; writer failure while A remains current -> original failure is semantic primary; S34-P62-06 keeps its existing identity primary plus writer compensation. Preserve exact B bytes, do not grant authority over B, and restore guard/permit/authority diagnostics in every applicable row.
- [x] 1.4 At the pre-invocation seam for `recoverCompletedRecordAfterRollbackFailure`, prove separately: unchanged A -> returned/replayed completed record with requested `result_ref`; same-digest valid completed B -> returned/replayed B; different-digest B -> `TaskServiceError.code=idempotency_mismatch`; missing/unsafe-result completed B -> `TaskServiceError.code=record_malformed`; byte-distinct same-digest started or failed B, including a canonical-equal raw-byte/physical replacement -> retryable `TaskServiceError.code=record_malformed` with status 409; malformed bytes B -> `TaskServiceError.code=record_malformed`; missing successor -> existing missing-transition `record_malformed`; writer failure with A current -> original failure is semantic primary. Preserve exact B bytes, perform no mutation of B, and restore diagnostics in every applicable row.
...
- [x] 1.6 Prove the Child A diff leaves `workspace-record-store.ts` unchanged and contains no service-side physical-generation inference for store-internal failures. Record #107 as the implementation-ready follow-up for internal publication-window total classification.
