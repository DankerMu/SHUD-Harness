...
- [x] #183.B Prove the seam cannot grant ambient authority.
  ...
  - Binding-aware structural traversal resolves every `BindingName`, excludes aliases/projections, requires the sole top-level mediation helper and each resolved raw callable, and binds all five exact operation callbacks, operands, results, and lifecycle relations. Copied-production-tree identifier, object/array destructuring, renamed-property, property-projection, assignment, bind, wrapper, nested-function alias mutations for every primitive, plus four Node primitive callback-escape mutations, each make the proof red.
