...
- [x] #183.A Add the exact primitive mediation seam.
  - In: `contracts/lib/capabilities.ts` and focused descriptor mediation tests only. The sole new runtime export is `installDescriptorPrimitiveMediator`; the sole new type exports are `DescriptorPrimitiveInvocation = () => unknown` and `DescriptorPrimitiveMediator = (operation: DescriptorOperation, invoke: DescriptorPrimitiveInvocation) => unknown`. Keep the registry, descriptor records, `ContractCapabilities` implementation, raw callables, raw results, and any authority-enter/reset/getter/replacement path private.
  - The installer validates before its module-instance latch, is frozen and non-constructible, and exposes only its standard own data surface. The mediator receives the exact `DescriptorOperation` plus a synchronous, callback-scoped, exactly-once closure only around `openSync`, an already-resolved `openat` callable, `fstatSync`, `readSync`, or `closeSync`; lazy `dlopen`/symbol resolution finishes before the callback. Callable metadata reflection is forbidden.
  - One shared `inactive|callback` state rejects every reentrant public `ContractCapabilities` entry and denial path with `CONTRACT_CAPABILITY_PRIMITIVE_MEDIATOR_REENTRY` before validation, lifecycle, hooks, or raw work. `invoke()` returns no raw result to the mediator; once its first raw primitive starts, that raw return/error wins over post-invoke throw, thenable, or caught/uncaught repeat. Omission/ordinary thenable/deferred use fails stable with zero raw calls. A pre-raw `close_sync` failure restores retryability; an attempted raw close remains terminal under existing precedence.
- [x] #183.B Prove the seam cannot grant ambient authority.
  - Process-isolated tests cover default behavior; invalid→first-valid→second-valid installation; frozen/non-constructible installer descriptors and source mutation; exact operation order; every `openSync` count and argument tuple plus a non-root-open mutation; callable Proxy mediation; raw-result non-capture; same-fd close/reopen replacement; post-invoke throw/thenable/raw-error precedence; caught and uncaught repeat; omitted, ordinary-thenable, async, late, and deferred invocation; and omitted/pre-invoke close retry settlement.
  - The fixture table proves invalid root/phase, parent/flags, stat handle, read phase/range, and close owner invoke neither mediator nor raw primitive. The reentry fixture covers `sealAdmission`, both opens, `markRetained`, `stat`, `readRetained`, `close`, `rejectForbidden`, and the denial boundary. Binding-aware structural traversal rejects local same-name helper and conditional `openAt` shadows. Existing four `CapabilityHooks` proof remains, while real `readBoundedFile` and `runCheckForTest` prove `afterAdmission`, `observe`, and `beforeCleanup` callbacks enter inactive and callback-started eligible work opens a distinct inactive-prior window.
...
- [x] #183.C Verify and hand off the prerequisite.
  - Source-only red proof stashes only `contracts/lib/capabilities.ts` while retaining the focused tests, runs the focused command once, restores immediately, and requires every new behavior assertion to fail against pre-fix source. The red fixture is process-isolated; no source/test mutation may remain after restoration.
  - Focused Darwin command: `npx --yes bun@1.2.19 test spikes/git-status-capability/contracts/tests/authority-descriptor-structural.test.ts spikes/git-status-capability/contracts/tests/authority-descriptor-mediation.test.ts spikes/git-status-capability/contracts/tests/source-ingress.test.ts`; type proof: `npx --yes bun@1.2.19 x tsc -p spikes/git-status-capability/contracts/tsconfig.descriptor-authority.json`.
  - Focused Linux command mounts `REPO` read-only at `/repo` and `DEPS` at sibling `/node_modules`, sets `NODE_PATH=/node_modules`, and runs the same three test files plus `/node_modules/.bin/tsc -p spikes/git-status-capability/contracts/tsconfig.descriptor-authority.json`; it MUST NOT mount dependencies at `/repo/node_modules` under the read-only source parent.
