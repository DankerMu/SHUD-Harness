...
- [x] 1.1a Freeze canonical source ingress, source identity, and the exact synthetic oracle (#164).
  ...
  - Named evidence: `source-ingress.test.ts` covers UTF-8/surrogates/canonical bytes, CR/LF path rejection, public source byte/depth/item exact/+1 behavior, and isolated source/metadata parser-node exact/+1 behavior with only the item ceiling relaxed; `source-identity.test.ts` covers the valid four-peer SHA fixture plus every independent/synchronized strict-subset forgery; `synthetic-oracle.test.ts` independently constructs the three-entry vector, rejects CR/LF paths, and rejects synchronized truncated/same-length frame+sidecar pairs; `current-source-authority.test.ts` proves exact manifest/filesystem/index equality, complete v2/v3/v4 entry grammar, normal/linked SHA-1/SHA-256 config and repository-extension authority, descriptor-bound no-follow worktree reads, exact receipts twice, byte-identical tracked/untracked status before/after, no created path, and no process launch. The SHA/source-blob-bound batched source-only replay is persisted in `.workplans/pr-167/review/red-proof-round-3-invariant-closure.md`. Compatibility fixtures expose only source identity fields from future platform/decision vocabulary and derive no state or decision.
