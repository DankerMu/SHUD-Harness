...
- [x] #183.A Add the exact primitive mediation and cleanup-ownership seam.
  ...
  - A private nonexported retryable-close error classification identifies no-raw close failure. Root/child rollback, verification cleanup, retained final cleanup, and checker ingress retry only that result, through the mediator and at most twice total; raw-terminal close stays terminal. After a second persistent no-raw refusal, retain every still-live capabilities/descriptor/owner pair strongly and poison the module-instance ingress boundary. The finite first chain may complete logical cleanup but cannot begin a third attempt or raw close for any retained owner; all later direct/checker admission fails with existing `CONTRACT_SCHEMA_INVALID` before `openRoot` or fd allocation, preserves primary-over-cleanup precedence, has no unmediated fallback, and cannot grow retained ownership.
- [x] #183.B Prove the seam cannot grant ambient authority.
  - Process-isolated tests cover the five-operation return getter/Proxy matrix; five mutable raw-error channels with mediator try/catch/mutation attempts, exact caller outcomes, callback-active exactly-once receipts; frozen installer own/inherited descriptors and pre-freeze prototype mutation; constructor/tag/prototype/branding traps; invocation own-key, descriptor, prototype, symbol, and raw-result privacy across all five raw result kinds; exact frozen invalid-denial rows; every public-entry reentry in all five outer windows; and bounded ingress close settlement for transient and persistent refusal, including root/child/verification/retained owners, repeated direct/checker fail-stop, Darwin `/dev/fd` and Linux `/proc/self/fd` non-growth, and zero raw fallback.
  - Binding-aware structural traversal resolves every `BindingName`, excludes aliases/projections, raw imports/loaders, caches, resolvers, and cross-library helpers, requires the sole top-level mediation helper and each resolved raw callable, and binds all five exact operation callbacks, operands, results, and lifecycle relations. Copied-production-tree identifier, object/array destructuring, renamed-property, property-projection, assignment, bind, wrapper, nested-function alias mutations for every primitive; raw renamed-import/namespace/`require`/builtin/dynamic/cache-resolver/cross-library mutations; and four Node primitive callback-escape mutations each make the proof red.
