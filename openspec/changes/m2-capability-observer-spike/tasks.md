...
task 1.1a owns its strict declaration syntax and initial content; tasks
1.1b–1.1e and every task 1.2–5.1 that adds/removes/renames a covered candidate have implicit In permission
to regenerate only this file. Task 1.1a accepts their canonical declarations
without asserting path existence or live equality. Task 1.1c (#166) owns
the exact Git tracked-set/repository authority and the future live-manifest equality
...
- [x] 1.1a Freeze canonical source ingress, source identity, and the exact synthetic oracle (#164).
  ...
  - Depends on: none.
  - Verification: one batched source-only red proof, then `npx --yes bun@1.2.19 test spikes/git-status-capability/contracts/tests` green. Public-seam cases cover ill-formed UTF-8; escaped lone/reversed/mismatched surrogates; a valid paired surrogate; byte exact schema-valid success; depth/item exact continuation to strict schema validation (`CONTRACT_SCHEMA_INVALID` is permitted because the synthetic boundary document is not a source authority record); depth/item +1 matching limit codes; isolated parser-node exact/+1 for the unchanged source and metadata node ceilings with only the dominated item ceiling relaxed; exact error/success receipts; independently and synchronously forged `source_sha`/platform `source_commit`/decision `base_sha`; the exact 152-byte three-entry synthetic vector; synchronized 58-byte truncation and same-length mutation with recomputed sidecars; entry count/order/path/mode/content/framing/digest mutations; and byte-identical repeat. Because the frozen counting rules make `nodes = items + 1`, real source/metadata profiles reach their item ceiling first; the node ceiling remains defense in depth and is not claimed as an independently reachable public real-profile boundary. The current checker validates only the strict declared manifest and committed metadata/frame/sidecar oracle, emits one deterministic success receipt without writes, and establishes no live repository, tracked-set, mode, config, object-format, or filesystem-generation authority. No Git helper, launcher, observer, or `verify.sh` command runs.
  ...
  - Named evidence: `source-ingress.test.ts` covers UTF-8/surrogates/canonical bytes, CR/LF path rejection, byte exact success, depth/item exact continuation to schema validation, matching +1 limit codes, and isolated source/metadata parser-node exact/+1 behavior with only the item ceiling relaxed; `source-identity.test.ts` covers the valid four-peer SHA fixture plus every independent/synchronized strict-subset forgery; `synthetic-oracle.test.ts` independently constructs the three-entry vector, rejects CR/LF paths, and rejects synchronized truncated/same-length frame+sidecar pairs; `current-source-authority.test.ts` proves strict UTF-8/LF/canonical sorted-unique manifest declarations containing the three mandatory committed oracle paths, bounded descriptor/no-follow reads and replacement rejection, frozen metadata/frame/sidecar mutation rejection, exact receipts twice, byte-identical status before/after, no created path, and no process launch. It makes no Git config/index/tracked-set/mode/filesystem-generation claim. Compatibility fixtures expose only source identity fields from future platform/decision vocabulary and derive no state or decision.
  - Full evidence floor: focused tests and all three public commands above; `npx --yes bun@1.2.19 run check`; `npx --yes @fission-ai/openspec@1.3.1 validate m2-capability-observer-spike --strict --no-interactive`; `test "$(git merge-base f8b74e724dc978acb889f715a936feabfd69680d HEAD)" = f8b74e724dc978acb889f715a936feabfd69680d`; `git merge-base --is-ancestor f8b74e724dc978acb889f715a936feabfd69680d HEAD`; `git diff --check f8b74e724dc978acb889f715a936feabfd69680d...HEAD`; `git diff --name-only f8b74e724dc978acb889f715a936feabfd69680d -- . ':(exclude)spikes/git-status-capability/contracts/**' ':(exclude)openspec/changes/m2-capability-observer-spike/**' ':(exclude).review-gate-issues.json' ':(exclude).workplans/pr-167/**'` with empty output, where the last two exclusions are review-only evidence and never source/runtime/package/workflow authority; `git diff --submodule=short --exit-code f8b74e724dc978acb889f715a936feabfd69680d...HEAD -- SHUD rSHUD AutoSHUD zero`; before/after `git status --porcelain=v1 --untracked-files=all` byte equality; and `test -z "$(git stash list --format='%gd %s' | rg 'red-proof')"`. The checker launches no child/helper process.
...
- [ ] 1.1c Bound Git authority and freeze the 17-gate execution profile (#166).
  - PR boundary: stable-workspace Git executable/exec-path/repository/tracked-set/mode/config/object-format authority, future live-manifest exact equality, bounded process lifetime, exact 17-gate order, and `GATE-UNTRACKED` profile with public-seam fixtures/tests only.
