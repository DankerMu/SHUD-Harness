...
- [x] #172.A Freeze and enforce the structural authority vocabulary.
  - In: `contracts/tests/{authority-vocabulary.ts,authority-structural.test.ts,authority-topology.ts}` only; registry `shud.contract.authority-proof.v2` owns every compile-valid mutation row and structural oracle, with exactly 55 ordered `(ID, control, structural violation, denial operation/target, side-effect oracle)` tuples bound to SHA-256 `8ae389ead0f1aaad27cdeb080f66e1841376552a963ef9069657d929a118a725` independently in both proof modules.
  - Out: core ingress/schema changes, general sandboxing, network controls, production imports, and runtime interception.
  - Verification: the unchanged `contracts/{check.ts,lib/**}` tree passes exact import/global/Object/element-access/binding baselines; every registry row is injected into a production module, compiles under Bun 1.2.19, and is structurally rejected with no preload. Rows cover direct/global/imported/cached `node:worker_threads` and bare `worker_threads` Worker routes; `eval`, direct `Function`, reflective descriptor, dynamic-key, destructured, object/function/arrow/async/generator/async-generator constructor acquisition; `Object.getPrototypeOf(...).constructor` variants; `import.meta["require"]`; dynamic import, `createRequire`, cached/computed builtins, FFI, child process, writes, and PathLike inputs. The structural proof separately parses the exact preload/control harness without executing it, permits OS/realm delegate calls only inside their named helpers, binds denial/delegate and close/exit/cleanup ordering, and rejects direct helper or lifecycle-wait bypass mutations.
- [x] #172.B Close the active runtime authority tripwire.
  - Depends on: #172.A.
  - In: `contracts/tests/{authority-vocabulary.ts,authority-runtime.test.ts,authority-control.ts,authority-preload.ts,authority-worker.ts}`; B consumes but does not redefine A's registry, and owns raw-delegation state beneath named real-delegate helpers plus bounded admission message-configured Worker-liveness canaries.
  - Verification: every registry control actually triggers after admission with structural scanning absent; direct/global/imported/cached Node and bare Worker plus every constructor-derived executable form is invoked, async-generator bodies are advanced with `.next()`, and denial occurs before Worker entry, ambient read, FFI load, child start, or write. Every denial row and direct success reports zero raw events; bounded read/FFI inversion canaries arm only their matching mode, invoke the same patched public wrapper/delegate as hostile rows, deliberately record raw delegation before the same denial, close any FFI library, clear the mode in finally, and clean their temporary root. A separate admission-phase sequence of message-configured Worker canaries passes no `workerData`; it runs sequentially over direct global, `node:worker_threads`, and bare `worker_threads` routes using only the fixed `authority-worker.ts` URL argument. The host attaches receipt/error listeners, sends an immediate `probe` plus short bounded retries through the constructed Worker's `postMessage`, and sends input/sentinel/channel exactly once only after the fixture's same-channel matching ready reply; wrong or duplicate ready replies cannot certify a route. The fixture receives and replies on that same channel with exact `transport: "message"` receipts. Before cleanup or the next route, the global route registers and awaits a bounded `close` event around `terminate()`, Node/bare routes boundedly await their promise/exit completion, and each receipt records exact termination plus completed sentinel removal. Existing Node/Bun/computed-loader/PathLike controls remain denied. The unique focused runner is `bun test contracts/tests/authority-structural.test.ts contracts/tests/authority-runtime.test.ts`; the full `contracts/tests` runner and both unchanged direct commands are green on Darwin and Linux.
...
- [x] #172.C Reconcile and persist exact proof evidence.
