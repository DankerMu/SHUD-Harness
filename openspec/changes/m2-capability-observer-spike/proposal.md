...
- The current Issue #176 delivery is the second #172 replacement child. It consumes only #175's frozen `CapabilityDescriptor`, `DescriptorCapabilityState`, `DescriptorOperation`, `DescriptorAuthorityDenial`, and immutable `DESCRIPTOR_OPERATION_POLICY` exports; binds every current Node/Bun/Worker/FFI/child real delegate to one lexical helper owner; and proves equivalent-call, alias, constructor, and shadow mutations fail source-only while the exact 55-row active registry and public receipts remain unchanged. Worker close/exit completion causality remains #177, and #178 owns only final tracked evidence publication.
