...
- [x] 8.1 [GRILL-3] zero `providers:` 配置（api_type/base_url/`api_key_ref: env:GLM_API_KEY`/fallback_chain + 按功能选模型占位）+ 连通冒烟脚本（非空 completion + 命中 base_url + exit 0；key 缺失明确报错；结论入 readiness notes）——依赖: 2.1
  - Fixture (#37): expanded / high
    ...
    - Invariant Matrix:
      ...
      - Producers: provider config file; fixed-authority canonical smoke CLI/API; explicitly noncanonical fixture engine with distinct filename and `evidence_scope=fixture`.
      ...
      - Failure paths/rollback/stale state: missing key/provider/CLI-preflight failure exits nonzero with a locally constructed structured message; failed smoke must not write or preserve a passing readiness note; owned final-entry symlink/hardlink is unlinked without following/mutating its external target; oversized regular note is removed/replaced before content read.
    ...
    - Evidence floor (#37):
      ...
      - Smoke self-test covers missing `GLM_API_KEY`, fake fetch success/failure, fake fetch timeout/AbortError, canonical authority override rejection, noncanonical fixture identity/root guard, CLI preflight stale-pass invalidation, oversized prior note, and secret-like unknown CLI arguments without network or secret leakage; HTTP failure uses a provider-body sentinel and proves result/readiness/stdout/stderr omit response body, status text, headers, and external exception text; timeout path proves the canonical fixed 15000ms maximum and at most one retry.
