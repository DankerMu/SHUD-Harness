  ...
  - Fixture (#37): expanded / high
    ...
    - Invariant Matrix:
      ...
      - Regression rows:
        ...
        - Alternate `--config` / API config path -> rejected and cannot mint canonical passing readiness evidence; an unsupported secret-like CLI argument -> fixed local error, with stdout/stderr/readiness omitting the raw argument and sentinel value.
    ...
    - Evidence floor (#37):
      ...
      - Smoke self-test covers missing `GLM_API_KEY`, fake fetch success/failure, fake fetch timeout/AbortError, alternate config rejection, and secret-like unknown CLI arguments without network or secret leakage; HTTP failure uses a provider-body sentinel and proves result/readiness/stdout/stderr omit response body, status text, headers, and external exception text; CLI failure proves raw argument text is omitted; timeout path proves bounded timeout plus at most one retry.
