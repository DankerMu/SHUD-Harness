...
- [x] 8.1 [GRILL-3] zero `providers:` 配置（api_type/base_url/`api_key_ref: env:GLM_API_KEY`/fallback_chain + 按功能选模型占位）+ 连通冒烟脚本（非空 completion + 命中 base_url + exit 0；key 缺失明确报错；结论入 readiness notes）——依赖: 2.1
  - Fixture (#37): expanded / high
    ...
    - Invariant Matrix:
      ...
      - Producers: provider config file; fixed-authority canonical smoke CLI/API with private co-located canonical writer and full-schema validation; explicitly noncanonical fixture engine with distinct filename and `evidence_scope=fixture`. No exported direct canonical writer, synthetic-pass seeder, or canonical backup/restore helper.
      ...
      - Public routes/entrypoints: no-override canonical smoke script CLI/package script/API; test-only fixture entrypoint is noncanonical by filename/schema/root guard.
      - Frontend/downstream consumers: #38 acceptance consumes the redacted readiness note and PR evidence only.
      - Failure paths/rollback/stale state: missing key/provider/CLI-preflight failure exits nonzero with a locally constructed structured message; failed smoke must not write or preserve a passing readiness note; unsafe local `workspace`/`readiness` ancestor symlink or non-directory entry and final symlink/hardlink are removed without following/mutating external targets; current-uid-owned ancestor mode is restored to owner rwx without touching sibling notes; a safe regular pass becomes a verified non-pass tombstone before best-effort unlink so Darwin delete ACL denial cannot preserve it; oversized/unreadable regular note is removed before content read or pass preservation; canonical tests never restore an old pass after a failed current invocation.
      ...
      - Regression rows:
        ...
        - Static export inventory -> canonical filename writer is private to canonical engine; shared core has no file-write authority; fixture module can write only fixture filename/schema; no exported helper seeds, backs up, or restores canonical pass; failing canonical test teardown leaves no old pass.
