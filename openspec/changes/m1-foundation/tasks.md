...
- [x] 8.1 [GRILL-3] zero `providers:` 配置（api_type/base_url/`api_key_ref: env:GLM_API_KEY`/fallback_chain + 按功能选模型占位）+ 连通冒烟脚本（非空 completion + 命中 base_url + exit 0；key 缺失明确报错；结论入 readiness notes）——依赖: 2.1
  - Fixture (#37): expanded / high
    ...
    - Risk packs considered:
      ...
      - Resource limits / large input / discovery: selected - canonical network call uses fixed bounded prompt/15000ms timeout/retry/response parsing, intentional no-read response exits cancel/abort under the same deadline, fixture timeout cannot exceed the same maximum, and stale readiness content is size-capped before read.
    ...
    - Invariant Matrix:
      ...
      - Producers: provider config file; fixed-authority canonical smoke CLI/API with private co-located canonical writer and full-schema validation; explicitly noncanonical fixture engine with distinct filename and `evidence_scope=fixture`. No exported direct canonical writer or exported synthetic-pass seeder.
      - Validators/preflight: canonical authority binding with no injectable root/config/fetch/env/clock/timeout; config parser/validator; missing-key check; URL normalization; nonempty completion assertion; response and stale-note bounded reads; CLI failure invalidation.
      ...
      - Regression rows:
        ...
        - Static export inventory -> canonical filename writer is private to canonical engine; shared core has no file-write authority; fixture module can write only fixture filename/schema; no exported helper seeds canonical pass.
