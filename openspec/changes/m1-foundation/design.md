...
6. **GLM provider 零开发接入**：zero `providers:` 块（`api_type: openai_chat_completions` + `base_url` + `api_key_ref` + `fallback_chain` + 按功能选模型）。`api_key_ref` 遵 [Config_Secrets §3](../../../docs/03_SPEC/Config_Secrets_And_Environment_Spec.md) SecretRef 形态（`env:GLM_API_KEY`，provider=env，purpose=llm；[GRILL-3] 定案 2026-07-03，Config_Secrets §4 已补行）。冒烟判定 = 最小 prompt 一次往返得到非空 completion 且实际命中配置的 `base_url`，exit 0。**2026-07-08 #37 执行裁剪**：DMXAPI 真实端点为 `https://www.dmxapi.cn/v1`；M1 连通 carrier model 先用 `deepseek-v4-pro-guan`（用户裁决，后续再替换为 `glm-5.2`），配置中保留 `target_model_id: glm-5.2` 与 SecretRef `env:GLM_API_KEY`。本机操作者可在 shell 中把已有 DMXAPI 环境变量临时映射给 `GLM_API_KEY` 运行冒烟；配置、日志、readiness notes 与提交产物只允许落 SecretRef 或变量名，不得落 secret 明文。
...
14. **TaskCard 幂等完成以可重放 snapshot 为 authority（2026-07-10 #74）**：同进程内、同 normalized workspace + scope + key 只允许一个共享 in-flight owner；entry 单独携带 owner request digest，同 key/different digest 在 durable record 发布前后均稳定 422，不能按 digest 分裂成两个 owner。相同 digest follower 在独立 5000ms 资源界内等待 owner 终态，再从 IdempotencyRecord + durable snapshot 重放；超过该界只让 follower 返回稳定可重试 409，仍活跃的 owner 不得删除、替换或复制，后续完成后仍由其结果重放。这个界同时保留“超过 1000ms 的合法慢请求不误判 409”验收与 fixture 的 bounded in-flight wait；无限 hook/IO 的 owner cancellation/lease 需要 supervisor，属于本项 non-goal。磁盘中只有 `started` 且无本进程活跃 owner 才走既有 bounded stale-claim 路径。`completeRecord` 前，snapshot producer 在 post-write hook 后必须以 bounded no-follow 直接读取比对精确 canonical durable bytes，并绑定 TaskCard/request digest；缺失、替换、malformed、unknown-field 注入或任意 byte/object drift 一律 fail closed，不能缓存 phantom TaskCard 或写 poisoned completed record。unsafe rollback 必须隔离 canonical snapshot leaf、驱逐 in-memory TaskCard，并保持 fresh hydration/修复后重试可用；若外部/并发路径在 producer 验证前提前写出 completed record，失败恢复必须验证其 result_ref authority，不合法时显式失效而非保留 poisoned completion。canonical TaskSnapshot 的 nested `task_card` 仍为可选载荷，但 M1 TaskCard 恢复需要该载荷；缺失时返回专门的 `task_snapshot_missing_card` recovery/migration error，不得伪装成 idempotency result binding mismatch。M1 单进程现实边界见 ADR-0002；跨进程 durable lease/supervisor 不在本修复范围。
...
- [M1 carrier model 临时非 GLM] → 2026-07-08 用户裁决：DMXAPI 当前先以 `deepseek-v4-pro-guan` 完成 OpenAI-compatible 通路冒烟，`glm-5.2` 替换作为后续模型切换债务；#37 不把 carrier 冒烟解释为 `glm-5.2` 模型准入。
