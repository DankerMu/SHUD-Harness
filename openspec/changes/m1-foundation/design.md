...
14. **TaskCard 幂等完成以可重放 snapshot 为 authority（2026-07-10 #74）**：同进程内、同 workspace + scope + key 的活跃请求由共享 in-flight 协调器收敛，后到的同 key 请求等待 owner 终态后再从 IdempotencyRecord + durable snapshot 重放；磁盘中只有 `started` 且无本进程活跃 owner 才走既有 bounded stale-claim 路径，不能把活跃慢请求误判成 pending 409。`completeRecord` 前，snapshot producer 在 post-write hook 后必须通过安全直接读取重新验证 leaf、TaskCard 与 request digest；缺失、替换、malformed 或不匹配一律 fail closed，不能缓存 phantom TaskCard 或写 poisoned completed record。canonical TaskSnapshot 的 nested `task_card` 仍为可选载荷，但 M1 TaskCard 恢复需要该载荷；缺失时返回专门的 `task_snapshot_missing_card` recovery/migration error，不得伪装成 idempotency result binding mismatch。M1 单进程现实边界见 ADR-0002；跨进程 durable lease/supervisor 不在本修复范围。
