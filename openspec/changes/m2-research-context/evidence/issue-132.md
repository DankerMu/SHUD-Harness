...
- Producers: superproject inventory/blob reader；four serial descriptor-bound checkout top-level/HEAD/branch/immutable-dirty-snapshot observers plus recursively discovered initialized stage-0 nested observers；two post-freeze complete publication sweeps；one unified final observable-identity recheck.
- Validators/preflight: physical root identity, exact Git top-level and empty prefix, 40-hex HEAD, bounded single-line branch, canonical Git booleans, safe config/ignore/text-conversion capture, standalone/split-index dependency capture, helper-free porcelain dirty boolean, strict schema parse.
...
| Resource limits / discovery | HEAD/branch empty, unterminated, multi-line, non-UTF-8, or above 64 KiB；real status above bound；standalone/split index or safe auxiliary input missing/replaced/oversized/drifting；Git timeout | invalid/maxBuffer output → `git_output_invalid`；timeout/nonzero → `git_read_failed`；status empty → clean and one/many porcelain records → dirty；four fixed published repos plus initialized nested recursive reads，with collection-global recursion budgeting deferred only to #134 |
...
- Read surfaces: exact four declared published checkout roots plus recursively discovered initialized stage-0 nested checkouts，all through per-command/per-file bounds；#134 remains the sole owner of collection-global depth/entry/command budgets.
