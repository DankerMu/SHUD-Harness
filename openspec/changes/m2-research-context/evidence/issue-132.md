...
| Concurrency / ordering | each early repo drifts after first-sweep read as next sibling begins；three early repos mutate commit/branch/dirty after `statusReads===8`；checkout identity changes through two publication sweeps；swap-use-restore；first sibling failure | first-sweep drift is caught by second-sweep/final identity；two completed maps must match；post-second-sweep-final-logical mutation is an explicit exclusion；transient swap cannot redirect reads/publish target；post-final-identity/fully unobserved ABA excluded；no partial repo map or orphan producer |
...
- Publication evidence now distinguishes first-sweep→second-sweep fallback from the public exclusion boundary. The former three early repos × commit/branch/dirty/identity remains caught；the new three early repos × commit/branch/dirty rows mutate only after `statusReads===8` and intentionally demonstrate that a stable return is permitted after that repo's second-sweep final logical observation. No third sweep was added；same-path replacement still cannot redirect worktree or object reads.
- Focused collector = `73 pass / 0 fail / 237 expect`；dirty-state = `95 pass / 0 fail / 299 expect`；required two-file batch = `168 pass / 0 fail / 536 expect`。Attached/detached tests each carry a 30s bound；the exact attached selection with `--rerun-each 12` produced `11 pass / 0 fail` actual executions, exceeding the ten-execution floor.
- `test:schemas` = `39 pass / 0 fail / 188 expect`；`test:core-services` = `683 pass / 5 skip / 0 fail / 30392 expect`；`typecheck`、schema generator self-test/drift check、strict OpenSpec、docs self-test、343-file link scan、`git diff --check`、stash/temp-marker hygiene passed。Full aggregate `check`、backend and PERF remain orchestrator-owned per the assigned boundary.
- The local source-only batched RED restored the pre-Round3 collector while retaining final tests and produced named semantic failures for post-audit injection, all six worktree-config rows, special-path recursion, and conflict/malformed parsing；the pre-existing clean nested fsmonitor row passed and is honestly retained as regression-only. The committed replay is `issue-132-round-3-red-proof.sh`: it binds five committed blobs, applies three semantic-mutant groups, requires exactly `0 pass / 15 named fail` then `15 pass / 0 fail` twice, rejects harness failures, and must be run by the orchestrator only after commit. No future SHA/blob/result is fabricated.
