...
- `packages/core/src/domain/services/stack-lock-collector.ts`: collect and revalidate four gitlink revisions, declared branch authority, bounded `renv.lock` state, runtime placeholders, harness identity, and the safe provider projection.
...
- The default Git child uses a minimal non-secret environment, disables lazy fetch, trace sinks, config inheritance, prompts, replacement objects and optional locks, and cannot fetch or write Git/trace state.
- Existing `renv.lock` is limited to 16 MiB at the opened descriptor; cheap bounded producers must succeed before hashing starts.
...
- The non-barrel process seam proves default-runner timeout/maxBuffer mapping and a minimal non-secret child environment. Local promisor and trace-sink fixtures prove no lazy fetch, object-pack growth, or arbitrary trace-file write. Real repository collection proves the four actual gitlinks, declared branches and zero pin while comparing HEAD and complete tracked/untracked status before/after.
...
- Producers: fixed minimal-environment `git ls-tree` command, bounded repository JSON/`.gitmodules` readers, descriptor-bounded shared `hashFile`, internal runtime placeholder projection, and deterministic digest constants.
- Validators/preflight: exact mode/type/path/count gitlink parser, 40-hex commits, exact four `.gitmodules` path/branch declarations, two-snapshot source identity comparison, repository path safety + durable single-link reads, required harness version, canonical provider/model selector consistency, safe HTTP(S) base URL, and StackLock content projection parse.
...
- Read surfaces: fixed superproject Git tree; fixed package/provider JSON and `.gitmodules`; optional fixed, 16 MiB-bounded `renv.lock`.
...
- Focused collector tests: injected/default Git command, minimal child environment, local no-lazy-fetch/trace fixtures, exact `.gitmodules` authority, missing/existing/symlink/bounded renv, provider secret/URL rejection, real four-gitlink/declared-branch/zero-pin and git-mutation guard.
...
- Provider/API/general credentials and every inherited `GIT_TRACE*` sink are absent from the child environment; `GIT_NO_LAZY_FETCH=1` prevents promisor fetch/object writes.
- Stable inputs produce matching full snapshots; gitlink, package, provider, `.gitmodules`, or renv content/presence transition between legal observations -> `collection_state_changed` with no partial result.
- `hashFile` exact `maxBytes` succeeds; bound+1 fails from the opened descriptor before content reads. The collector maps a 16 MiB+1 `renv.lock` to `renv_lock_invalid`, and cheap producer failure starts no renv hash.
...
- Round 2 repair source-bound replay: retaining the new regression rows while restoring the pre-repair production sources made 15 new behavior assertions fail; exact sources were restored immediately with no stash residue.
