...
Historical evidence note: this committed `f493e77235acbe26ff3f8587192a9eab32efa77e` proof is retained unchanged for its selected Round 2 claims, but Round 3 supersedes its preflight-only filter coverage with `issue-132-round-3-red-proof.{md,sh}` for post-audit injection, effective worktree config, and strict nested-index parsing. The Round 2 SHA, blobs, counts, and observed output below remain historical facts and are not presented as Round 3 evidence.
