...
- [x] 4.1 采集服务：submodule gitlink 只读发现（四 repo commit/branch，W2-SUB-001）+ renv.lock 内容哈希（缺失 → null + `renv_lock_missing` 降级）+ runtime 版本占位 + harness 块占位口径（version=根 package.json，cli_version/prompt_pack/skills_version=占位常量，design D7a）+ llm 块从 provider 配置读取 provider/model_id/base_url、params_digest/prompt_pack_digest 按 D7a 语义分别计算（依赖: 2.1、3.1。验证: zero commit == pin 断言；harness 占位断言；两 digest 确定且互不相同；api key 值零出现）
  - Fixture (#91): fixture level = expanded；repair intensity = high；public core service、固定 Git 子进程、repository/config 文件读取、credential-adjacent provider projection、四 submodule/zero 复现证据触发 mandatory expanded；完整 fixture 与执行偏离见 `evidence/issue-91.md`。
  ...
  - Boundary-surface checklist (#91): collector/test/barrel、root canonical `0.8.3` metadata plus OpenSpec evidence；consume but do not modify StackLock schema、hashing、path safety、durable reader、provider config、dependency lock、routes、record store or submodules；zero runtime write/delete/overwrite/persist surface。
  - Evidence floor (#91): exact injected Git args + missing/duplicate/wrong-mode inventory；default Git timeout/max-buffer mapping、hostile environment sanitation、malformed/oversized output -> typed non-disclosing failure with zero partial output；two full source snapshots match before publish，generation drift typed fail；oversized package/provider JSON and replacement/read drift -> typed non-disclosing failure with zero partial output；real four-gitlink/zero-pin and full HEAD/index/tracked/untracked no-mutation guard；missing/existing/change-sensitive/symlink renv；required harness version、canonical provider selector/model、digest oracles；API-key/path non-disclosure；source-bound red proof；`test:core-services`、`typecheck`、`check`、`schema:check`、`test:perf:api`、strict OpenSpec、frozen install、DependencyLock、diff/lock/submodule/workspace hygiene。
