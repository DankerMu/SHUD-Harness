...
- [x] 1.1a Child A — workspace file-backed local-token store production module（不接线 routes）：0600/0700、bounded UTF-8 token、descriptor-relative no-follow、exact-generation no-clobber publication/recovery、稳定目录资源预算、全失败路径 descriptor 收敛；完整 expanded/high fixture 见 design「Issue #87 Child A」（依赖: 0.1。验证: source-bound semantic red + coverage-only 账；macOS/Linux focused；backend compatibility；typecheck/check/perf/OpenSpec/hygiene）
