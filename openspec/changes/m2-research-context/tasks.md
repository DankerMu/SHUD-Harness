...
- [x] 2.1 StackLock + DataProvenance + ArtifactManifest Zod schemas + Artifact `llm_generated` 扩展 + 正反例单测（strict 拒废弃字段；llm.base_url required；event_window 对象形态；ID 格式 `STACK-/DATA-/MANIFEST-<uuid>`（design D5）；StackLock repos 四键与 `r_packages_lock` 对象形态按 D5a 偏离记录；ArtifactManifest 与 Support_Schema_Contracts §2 一一对应——含必填 `generator`、可选 `report_id`、`artifacts` 为完整 Artifact[]、可选 `superseded_by`；Artifact 增 `llm_generated` 可选缺省 false（design D6 偏离记录）（依赖: 0.1。验证: `bun run test:schemas`、schema drift 检查覆盖 D5a/D6 偏离记录）
  - Fixture (#89): expanded / high；新增公共 Zod 对象契约、strict unknown-key 语义、默认值输入/输出差异与生成 JSON Schema/Markdown 触发 mandatory expanded，沿用本 change 的 proposal/design/spec 全集。
  - Invariant Matrix (#89): canonical source = Minimal_Schemas §2/§3 + Support_Schema_Contracts §1/§2 + OpenSpec deltas；producers = 四个 `packages/core/src/domain/schemas/*` exports；validators = prefixed UUID、四 repo exact shape、object event_window、完整 Artifact、required llm.base_url/generator；storage = none；consumers = 4.x/5.x/6.x 后续 services；failure = 缺字段、废弃/未知字段、旧 ID/窗口/renv 简写均 parse fail；evidence = schema tests + generated JSON/Markdown drift。
  - Risk packs (#89): Public API、Schema / fields、Legacy compatibility、Error handling、Release / packaging、Documentation / migration、Scientific evidence lineage selected；Config、File IO、Auth、Concurrency、Resource limits、Hydrology runtime、Zero governance not selected（无对应变更面）。
  - Boundary-surface checklist (#89): 只改 core schema exports/tests、schema generator registry 与 `docs/generated/**`；不接线 service/route、不创建 workspace 写路径、不改变 TaskCard/error/idempotency/lock 既有对象语义；strictness 仅施加于本任务新增对象及 Artifact delta。
  - Evidence floor (#89): StackLock 四 repo + nullable/object renv 正例，missing base_url、旧 renv 字符串、runtime.container/root limits/policy_version/degraded 负例；DataProvenance object window/hash/source 正例与 array window/缺 hash/缺 observation field/unknown key 负例；ArtifactManifest full Artifact/optional refs 正例与 compact artifact/missing generator/invalid IDs 负例；Artifact omitted marker → false、true 保留、非 boolean/unknown key 拒绝；`npx --yes bun@1.2.19 run test:schemas`；`npx --yes bun@1.2.19 run schema:check`；`npx --yes bun@1.2.19 run typecheck`；`npx --yes bun@1.2.19 run check`；`npx --yes @fission-ai/openspec@1.3.1 validate m2-research-context --strict --no-interactive`；`git diff --check`；package/lock/submodule/workspace hygiene。
