...
- [x] 1.1a Child A — workspace file-backed local-token store production module（不接线 routes）：0600/0700、bounded transport-safe UTF-8 token、descriptor-relative no-follow、final-name no-clobber cooperative bootstrap（无随机目录-staging residue）、typed mutation-lease 下的 lease-serialized generation-validated token publication/recovery、稳定目录资源预算、全失败路径 descriptor 收敛；不承诺对忽略 bootstrap/ mutation-lock 协议的同目录 writer 提供跨平台 source-inode-conditioned rename/unlink；完整 expanded/high fixture 见 design「Issue #87 Child A」（依赖: 0.1。验证: cooperative absent-tree concurrency + post-mkdir process-death restart、source-bound semantic red + coverage-only 账；macOS/Linux focused；backend compatibility；typecheck/check/perf/OpenSpec/hygiene）
