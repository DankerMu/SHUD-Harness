...
采集先把调用方 `repositoryRoot` 物理规范化，再以同一最小非敏感 Git seam 执行 `git --no-lazy-fetch rev-parse --show-toplevel`；Git 报告的物理 top-level 必须与请求根完全相等，且该身份在两次 cheap snapshot 中重读并比较。任一不等（包括指向同仓库 nested directory）在读取 package/provider 或运行 authority inventory 前以稳定 typed error fail closed。随后以单个 bounded `git --no-lazy-fetch ls-tree -z --full-tree HEAD -- .gitmodules SHUD rSHUD AutoSHUD zero` 同时读取四个 gitlink commit（W2-SUB-001）与 mode=`100644` 的 `.gitmodules` blob id，再以 fixed `git --no-lazy-fetch cat-file blob <exact-object-id>` bounded 读取该精确 HEAD object；branch 只从此 blob 的 exact 四项声明读取（SHUD/rSHUD/AutoSHUD=`master`，zero=`development`），不读取工作树 `.gitmodules`。完整 authority identity（gitlinks、blob id、blob digest、declarations）纳入两次 cheap snapshot 比较。所有生产 Git argv 都把全局 `--no-lazy-fetch` 放在 subcommand 前，Git <2.45 不支持该选项时须在首条 top-level 命令 fail closed，不降级执行；不进入 submodule 工作树、不改任何 git 状态。runtime 版本（r/python/sundials/gcc/gdal）M2 填占位探测值或 `unknown`（Phased_Plan 明示"runtime versions 占位"）。llm 块从 provider 配置（#37 已落）读取 provider/model_id/base_url。现有 `renv.lock` 在打开后的 regular descriptor 上执行 inclusive 16 MiB 上限，超限 fail closed。
