...
session_id
task_id
agent_role          # coordinator | repo_explorer | worker | coder | reviewer
phase:              # brief | explore | plan | execute | collect | report | review
```

归因粒度为 phase 级别，不做单次 LLM call 级别拆分。典型用法：

- `plan` 阶段 token 占比过高 → 任务描述不够清晰，Coordinator 反复推理。
- `explore` 阶段 token 占比过高 → Repo Explorer 读取范围过宽，需要收紧目标仓库、文件模式或 max inspected refs。
...
## 10. CostMonitor 与 Observability 的关系
