...
**事实基线**：SHUD@3aec657（"v2.0 2022"，cpu-accel-v1.0.2）· rSHUD v2.5.0（228 导出函数）· AutoSHUD v2.5.0-rc（f421445）
...
- wb：`wb.all(xl=..., ic=readic(), fun=xts::apply.daily, plot=FALSE)`（WaterBalance.R:9-45，
  需要 elevprcp/eleveta/elevetp/rivqdown/eleysurf/eleyunsat/eleygw）→ 残差统计 + 阈值判定；
...
**版本门**：CLI 要求 `packageVersion("rSHUD") >= 2.5.0`（现代 API `read_output`/`load_output_data` 自 v2.5 稳定导出）；
低版本直接拒绝（exit 4），不做静默 legacy 回退——deprecated 包装器（readout/loaddata）不是本 CLI 的依赖面。
...
- `AUTOSHUD_SHUD_SOURCE` ← StackLock 锁定的 SHUD 路径（覆盖 Step4 源码解析链，Step4_SHUDRunner.R:229-252）；
- `AUTOSHUD_SHUD_BUILD_TIMEOUT` / `AUTOSHUD_SHUD_RUN_TIMEOUT` ← RunJob 超时配置（默认 600s/1800s）。
