...
**语言**: R 脚本集合 (非 R 包)
**快照声明 (2026-07-02)**: 本报告基于 1cbec6f；子模块已同步至 f421445 (≈v2.5.0-rc)。实际流水线为 **Step0.1(可选)+Step1-5 共 5-6 步**（非"7 步"），Step4 内置 SHUD 源码解析/构建/运行与超时控制，新增 acceptance 测试。步骤→对象映射见 [Domain_CLI_Spec](../03_SPEC/Domain_CLI_Spec.md) §6，本报告全量刷新待办。
