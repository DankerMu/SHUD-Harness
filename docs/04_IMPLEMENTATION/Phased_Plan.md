...
2. 一条治理规则端到端穿透（**2026-07-04 修订，ADR-0001 revisit 裁决 / 账本例外批次 5**）：路径写禁区（如 `data/raw/**`）的 authority 位于执行层 OS 沙箱——bash 工具 spawn 命令时施加沙箱 profile（macOS `sandbox-exec`/seatbelt，`deny file-write*` subpath，子进程继承），写入在 syscall 层按解析后真实目标路径被拒，合法 raw 读不受影响；pre-exec 静态检查降级 advisory（原"真实执行前被拒"表述经首轮 spike 判 Not green——静态扫描无法覆盖任意 shell 写入）。**拒绝错误体含 `remediation{next_action, hint, ref}`**（Control_Kernel §5 拒绝载荷约定），拒绝事件出 WebSocket 并落 AuditEvent（**2026-07-05 PR #48 gate 收窄**：拒绝遥测收窄为可信可观测——当前仅 sandbox tool 内层 advisory/static 同根 raw 写证据产 tool.failed/audit denial，外层 `RAW_DATA_WRITE_RULE_ID` evaluator denial 属配置误用；post-exec 进程输出/退出码只记录普通 lifecycle `failed`，不得单凭该结果声明 `denied_by_sandbox`；子进程吞拒绝 exit 0 的隐藏遥测与双 fork/setsid 后代进程所有权移出、归后续 executor/audit，raw 字节仍由继承 profile 守住；合法 waited 前台子进程保持放行）；
...
修订注为准（① 自建薄工具注册层 ② Claude Agent SDK 迁移），不带病继续。首轮判定（2026-07-04）：条 2
原形态不绿 → revisit 已触发并同日裁决——边界重划、基座不换（两备胎对该失败类无效），条 2 按上文修订执行，
条 3 及工具面治理任务解冻按原依赖图恢复；详见 [ADR-0001 revisit 记录](../adr/0001-agent-runtime-and-topology.md)
与实施记录 `openspec/changes/m1-foundation`。条 2' 首实现（PR #48）gate 复审进一步把遥测/进程所有权收窄为可信可观测边界（seatbelt 守 raw 字节不变，post-exec 进程结果不作 `denied_by_sandbox` 归因源，finding 未击穿完整性）——见 ADR-0001 2026-07-04 裁决补充 / 2026-07-05 gate 收窄与账本例外批次 5。
