...
import { COMMAND_PROFILE_V1, FROZEN_GATE_ENVIRONMENT } from "../lib/command-profile";
...
describe("frozen catalog v1 contract", () => {
  ...
  test("freezes four state layers, all limits, stable codes, and tool versions", async () => {
    ...
    expect(value.supply_identity).toEqual(SUPPLY_IDENTITY);
    expect(value.command_profile).toEqual(COMMAND_PROFILE_V1);
  ...
  test("freezes exact supply identities and the ordered two-stage untracked gate", () => {
    expect(SUPPLY_IDENTITY).toEqual({
      lockfile_digest: "0b464510a35a2812bdc3fd5960d98a350baa949019ce7181ef01a4eb8195c02a",
      rust_toolchain_digest: "2497367eeaf2f826dc39daeeb64d8782ea022abe73f2df98e64cd967f1f37fd5",
      rust_release: "1.88.0", rust_commit: "6b00bc3880198600130e1cf62b8f8a93494488cc",
      cargo_cli_release: "1.88.0", cargo_commit: "873a06493", cargo_package_version: "0.89.0",
      git_version: "2.49.0", targets: { macos: "aarch64-apple-darwin", linux: "x86_64-unknown-linux-gnu" }
    });
    expect(JSON.parse(JSON.stringify(COMMAND_PROFILE_V1.map((gate) => [gate.ordinal, gate.id])))).toEqual([
    ...
    expect(COMMAND_PROFILE_V1[12]!.stages).toEqual([
      { argv: ["git", "status", "--porcelain=v1", "--untracked-files=all"], tool: "git", version: "2.49.0", environment: FROZEN_GATE_ENVIRONMENT },
      { argv: ["spikes/git-status-capability/verify.sh", "untracked-inventory"], tool: "repository-script", version: "1", environment: FROZEN_GATE_ENVIRONMENT }
    ]);
  });
