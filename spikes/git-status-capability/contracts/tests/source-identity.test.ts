...
type IdentityPlatformFixture = {
  platform: string;
  source_commit: string;
  observer_outcome?: string;
  [key: string]: unknown;
};
type IdentityFixture = {
  source_record: { source_sha: string; [key: string]: unknown };
  platforms: [IdentityPlatformFixture, IdentityPlatformFixture];
  decision: { base_sha: string; terminal_decision?: string; [key: string]: unknown };
  run_status?: string;
  [key: string]: unknown;
};
...
describe("source identity projection", () => {
  test("the direct public command emits the exact LF-terminated receipt", async () => {
  ...
  test("every independent and synchronized strict-subset four-SHA forgery fails", async () => {
    const fixture = JSON.parse(await readFile(validIdentityPath, "utf8")) as IdentityFixture;
  ...
  test("missing, unknown, reordered platform, malformed SHA, and future vocabulary fail closed", async () => {
    const fixture = JSON.parse(await readFile(validIdentityPath, "utf8")) as IdentityFixture;
    const mutations: Array<(value: IdentityFixture) => void> = [
      (value) => { delete value.decision; },
      (value) => { value.run_status = "valid_complete"; },
      (value) => { value.platforms.reverse(); },
      (value) => { value.source_record.source_sha = "A".repeat(40); },
      (value) => { value.platforms[0].observer_outcome = "clean"; },
      (value) => { value.decision.terminal_decision = "accepted"; }
    ];
