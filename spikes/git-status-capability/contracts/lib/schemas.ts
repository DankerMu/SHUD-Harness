...
import { canonicalEqual, canonicalJson } from "./canonical-json";
import { SOURCE_METADATA_PROFILE, SOURCE_PROFILE, SYNTHETIC_DIGEST, SYNTHETIC_FRAME_BYTES } from "./constants";
...
function admittedSet(value: JsonRecord): boolean {
  ...
  if (!value.admitted_paths.every(relativePath) || !value.admitted_modes.every((mode) => mode === "100644" || mode === "100755")) return false;
...
const RESULT_KEYS = ["status", "source_input_digest", "manifest_digest", "entry_count", "admitted_paths", "admitted_modes"] as const;
function encoderResult(value: unknown): value is JsonRecord {
  return record(value) && exactKeys(value, RESULT_KEYS) && value.status === "ok" && sha256(value.source_input_digest) &&
    sha256(value.manifest_digest) && admittedSet(value);
}
...
export function validateSourceInputRecord(value: unknown): value is JsonRecord {
  if (!record(value) || !exactKeys(value, SOURCE_KEYS) || value.schema_version !== "shud.git-status-capability.source-input-record.v1" ||
      !gitObject(value.source_sha) || !sha256(value.source_input_digest) || !sha256(value.manifest_digest) || !admittedSet(value) ||
      !encoderResult(value.primary_result) || !encoderResult(value.witness_result)) return false;
  const projection = [value.source_input_digest, value.manifest_digest, value.entry_count, value.admitted_paths, value.admitted_modes];
  ...
  return value.primary_encoder === "source-input-primary-v1" && value.witness_encoder === "source-input-witness-v1" &&
    canonicalEqual(projection, primary) && canonicalEqual(projection, witness) && commandReceipt(value.command_receipt, value.source_sha as string);
}
...
export function validateSourceIdentityProjection(value: unknown): value is JsonRecord {
  if (!record(value) || !exactKeys(value, ["schema_version", "source_record", "platforms", "decision"]) ||
      ...
      !exactKeys(value.source_record, ["source_sha"]) || !gitObject(value.source_record.source_sha) || !Array.isArray(value.platforms) ||
      value.platforms.length !== 2 || !record(value.decision) || !exactKeys(value.decision, ["base_sha"]) || !gitObject(value.decision.base_sha)) return false;
  const expectedPlatforms = ["macos", "linux"];
  for (let index = 0; index < 2; index += 1) {
    const platform = value.platforms[index];
    if (!record(platform) || !exactKeys(platform, ["platform", "source_commit"]) || platform.platform !== expectedPlatforms[index] || !gitObject(platform.source_commit)) return false;
  }
  const peers = [value.source_record.source_sha, value.platforms[0]!.source_commit, value.platforms[1]!.source_commit, value.decision.base_sha];
...
export function validateContractMetadata(bytes: Uint8Array): void {
  const value = parseBoundedJson(bytes, SOURCE_METADATA_PROFILE);
  if (!record(value) || !exactKeys(value, ["schema_version", "source_profile", "synthetic_oracle"]) ||
      value.schema_version !== "shud.git-status-capability.source-contract.v1" || !record(value.source_profile) ||
      !exactKeys(value.source_profile, ["bytes", "depth", "nodes", "items"]) || !record(value.synthetic_oracle) ||
      !exactKeys(value.synthetic_oracle, ["frame_bytes", "entry_count", "sha256"])) throw new ContractError("CONTRACT_SCHEMA_INVALID");
  if (!canonicalEqual(value.source_profile, SOURCE_PROFILE) || value.synthetic_oracle.frame_bytes !== SYNTHETIC_FRAME_BYTES ||
      value.synthetic_oracle.entry_count !== 3 || value.synthetic_oracle.sha256 !== SYNTHETIC_DIGEST) throw new ContractError("CONTRACT_SCHEMA_INVALID");
}
