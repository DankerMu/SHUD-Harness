...
type InputOptions = { mode: "input"; input: string; kind: InputKind; repositoryRoot?: string };
...
function parseArgs(args: string[]): Options {
  ...
  if (!checkCurrent && values.has("--input") && values.has("--kind")) {
    const kind = values.get("--kind") as InputKind;
    if (!INPUT_KINDS.has(kind)) throw new ContractError("CONTRACT_SCHEMA_INVALID");
    const hasRoot = values.has("--repository-root");
    if ((kind === "authority_set") !== hasRoot || values.size !== (hasRoot ? 3 : 2)) throw new ContractError("CONTRACT_SCHEMA_INVALID");
    return { mode: "input", input: values.get("--input")!, kind, repositoryRoot: values.get("--repository-root") };
  }
